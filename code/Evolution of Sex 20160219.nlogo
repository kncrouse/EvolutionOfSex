...
to setup-vars
  ...
  set parasites-per-population 100
end
...
to setup-parasites
  ;let parent self
  create-parasites parasites-per-population
  [
    ;set parent-group parent
    initialize-parasite
  ]
...
to set-size-shape-color
  ...
  if (position ([color] of a11) allele-color-types) < (position ([color] of a12) allele-color-types) [
...
to-report initialize-color-allele
  ...
  hatch-alleles 1 [
    ...
    set hidden? true
    set shape "circle"
    set color one-of allele-color-types
    set new-allele self
...
to go
  ...
  ask parasites [ parasite-wander attempt-infection ]
  ask parasites [
    set age age + 1
  ...
  ask old-phenotype-group [
    if [infecting-parasite] of self = nobody [
      ;if count phenotypes < carrying-capacity + length old-phenotype-group [
        if [sex] of self = "asexual" [ ask self [ reproduce-asexually ]]
        if [sex] of self = "female" [ ask self [ reproduce-sexually ]]
      ;]
...
to parasite-wander
  let potential-host one-of phenotypes in-radius 4 with [ matching-parasite? myself ]
  if random-float 1.0 < parasite-infectivity [if potential-host != nobody [ face potential-host ]]
  right random 90
  left random 90
  fd 1
end
...
to-report matching-parasite? [ para ]
  let color-boolean [color] of para = (color - 2)
  let size-boolean [size] of para = (size / 3)
  report color-boolean and size-boolean
end
...
to reproduce-sexually
  ...
  let mate nobody
  if count eligible-males > 0 [ set mate one-of eligible-males ]
...
to-report get-eligible-males
  let eligible-males phenotypes with [ sex = "male" ]
